...
public class Tool_Mission_06 : MonoBehaviour
{
    protected eMissionType m_eMissionType = eMissionType.None;

    private InputField m_pInputField_Stripe = null;

    void Start()
    {
        m_eMissionType = eMissionType.Stripe;

        GameObject ob;

        ob = Helper.FindChildGameObject(gameObject, "InputField_Stripe");
        m_pInputField_Stripe = ob.GetComponent<InputField>();
    ...
    public void OnInputField_EndEdit(string strData)
    {
        ...
        {
            int nCount = Helper.ConvertStringToInt(m_pInputField_Stripe.text);

            if (SavedGameDataInfo.Instance.m_pMapDataInfo.m_pMapDataMission.m_MapDataMissionInfoTable.ContainsKey(m_eMissionType) == true)
            {
                MapDataMissionInfo_Stripe pMissionInfo = SavedGameDataInfo.Instance.m_pMapDataInfo.m_pMapDataMission.m_MapDataMissionInfoTable[m_eMissionType] as MapDataMissionInfo_Stripe;
            ...
            {
                MapDataMissionInfo_Stripe pMissionInfo = new MapDataMissionInfo_Stripe();
    ...
    private void OnUpdateMap(bool IsChangeMap)
    {
        if (SavedGameDataInfo.Instance.m_pMapDataInfo != null)
        {
            ...
            if (SavedGameDataInfo.Instance.m_pMapDataInfo.m_pMapDataMission.m_MapDataMissionInfoTable.ContainsKey(m_eMissionType) == true)
            {
                ...
                if (pMissionInfo != null)
                {
                    m_pInputField_Stripe.text = pMissionInfo.m_nCount.ToString();
                }
                else
                {
                    IsValid = false;
